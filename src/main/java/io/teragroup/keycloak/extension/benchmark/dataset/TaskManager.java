...
/**
 * Management of executed tasks and making sure that there won't be multiple
 * tasks in progress (EG. there is no creation of 1000 realms triggered
 * accidentally 2 times).
 * </p>
 * It stores its state in the {@link SingleUseObjectProvider} with static key so
 * that it is available across multiple nodes.
 *
...
public class TaskManager {
    ...
    /*
     * Previous versions of the provider stored their state in the workcache of
     * Infinispan which is part of the legacy store.
     * With that being no longer available after switching to the map storage, this
     * now uses the SingleUseObjectProvider to store a single value.
     * While this might not be a valid strategy for providers running a production
     * environment, this strategy is sufficient for running it in load testing
     * environments.
     */
