...
import static io.teragroup.keycloak.extension.benchmark.dataset.config.DatasetOperation.CREATE_AUTHZ_CLIENT;
...
public class DatasetConfig {
    ...
    // Used when creating many realms as a prefix. For example when prefix us "foo",
    // we will create realms like "foo0", "foo1" etc.
    // For many events, it will need the realm prefix as events are created randomly
    // in all the already created realms
    @QueryParamFill(paramName = "realm-prefix", defaultValue = "realm-", operations = { CREATE_REALMS, CREATE_EVENTS,
    ...
    // based on already created realms (clients, users)
    private Integer start;

    // Count of entities to be created. Entity is realm, client or user based on the
    // operation
    @QueryParamIntFill(paramName = "count", required = true, operations = { CREATE_REALMS, CREATE_CLIENTS, CREATE_USERS,
            CREATE_EVENTS, CREATE_OFFLINE_SESSIONS, CREATE_AUTHZ_CLIENT })
    private Integer count;

    // Prefix for realm roles to create in every realm (in case of CREATE_REALMS) or
    // to assign to users (in case of CREATE_USERS)
    @QueryParamFill(paramName = "realm-role-prefix", defaultValue = "role-", operations = { CREATE_REALMS,
            CREATE_USERS })
    ...
    // Prefix for newly created clients (in case of CREATE_REALMS and
    // CREATE_CLIENTS). In case of CREATE_USERS it is used to find the clients with
    // clientRoles, which will be assigned to users
    @QueryParamFill(paramName = "client-prefix", defaultValue = "client-", operations = { CREATE_REALMS, CREATE_CLIENTS,
            CREATE_USERS, CREATE_OFFLINE_SESSIONS, LAST_CLIENT })
    ...
    @QueryParamIntFill(paramName = "clients-per-transaction", defaultValue = 10, operations = { CREATE_REALMS,
            CREATE_CLIENTS, CREATE_AUTHZ_CLIENT })
    private Integer clientsPerTransaction;

    // default number of entries created per DB transaction
    @QueryParamIntFill(paramName = "entries-per-transaction", defaultValue = 10, operations = { CREATE_REALMS,
            CREATE_CLIENTS, CREATE_AUTHZ_CLIENT })
    private Integer entriesPerTransaction;

    // Prefix of clientRoles to be created (in case of CREATE_REALMS and
    // CREATE_CLIENTS). In case of CREATE_USERS it is used to find the clientRoles,
    // which will be assigned to users
    @QueryParamFill(paramName = "client-role-prefix", defaultValue = "client-role-", operations = { CREATE_REALMS,
            CREATE_CLIENTS, CREATE_USERS })
    private String clientRolePrefix;

    // Check if created clients should be service account clients
    @QueryParamFill(paramName = "client-access-type", defaultValue = "confidential", operations = { CREATE_REALMS,
            CREATE_CLIENTS })
    private String clientAccessType;

    // Check if created clients should be service account clients
    @QueryParamFill(paramName = "service-account-client", defaultValue = "true", operations = { CREATE_REALMS,
            CREATE_CLIENTS })
    private String isServiceAccountClient;

    // When creating clients, every client will have this amount of client roles
    // created
    @QueryParamIntFill(paramName = "client-roles-per-client", defaultValue = 10, operations = { CREATE_REALMS,
            CREATE_CLIENTS })
    private Integer clientRolesPerClient;

    // Prefix of groups to be created (in case of CREATE_REALMS operation) or
    // assigned to the users (In case of CREATE_USERS and CREATE_REALMS operations)
    ...
    // Count of realm roles assigned to every user. The roles assigned are not
    // random, but depends on the "index" of the current user and total amount of
    // roles available and assigned to each user
    @QueryParamIntFill(paramName = "realm-roles-per-user", defaultValue = 4, operations = { CREATE_REALMS,
            CREATE_USERS })
    private Integer realmRolesPerUser;

    // Count of client roles assigned to every user. The roles assigned are not
    // random, but depends on the "index" of the current user and total amount of
    // roles available and assigned to each user
    @QueryParamIntFill(paramName = "client-roles-per-user", defaultValue = 4, operations = { CREATE_REALMS,
            CREATE_USERS })
    private Integer clientRolesPerUser;

    // Password policy with the amount of password hash iterations. It is 20000 by
    // default
    @QueryParamIntFill(paramName = "password-hash-iterations", defaultValue = Pbkdf2PasswordHashProviderFactory.DEFAULT_ITERATIONS, operations = {
            CREATE_REALMS })
    ...
    // not be terminated immediatelly. However it will be permitted to start another
    // task
    // (EG. Send another HTTP request for creating realms), which can cause
    // conflicts
    ...
    // The number of users per user policy. If greater than 1, an aggregated policy
    // is created with the given number of user policies
