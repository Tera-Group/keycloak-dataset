...
public class AuthorizationProvisioner extends DatasetResourceProvider {
    ...
    public Response provision() {
        ...
        try {
            DatasetConfig config = ConfigUtil.createConfigFromQueryParams(uriInfo, CREATE_AUTHZ_CLIENT);
    ...
    private void provision(Task task, RealmContext context, KeycloakSession session, int resourceStartIndex,
        ...
        for (int currentResourceIndex = resourceStartIndex; currentResourceIndex < resourceEndIndex; currentResourceIndex++) {
            ...
            createScopePermission(scopeStore, resourceServer, scopeCount, scopePrefix, policyStore,
                    currentResourceIndex, resource, policies);
            context.incResourceCount();
        }

        task.debug(logger, "Created %d resources in client %s", context.getResourceCount(), config.getClientId());
        task.debug(logger, "Created resources in client %s from %d to %d", config.getClientId(), resourceStartIndex,
                resourceEndIndex);

        if (((resourceEndIndex - config.getStart()) / config.getClientsPerTransaction()) % 20 == 0) {
            task.info(logger, "Created %d resources in client %s", context.getResourceCount(),
    ...
    private boolean submitTask(RealmContext context, Task task) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    logger.infof("Will start creating resources in client '%s' from '%s' to '%s'", config.getClientId(),
                            config.getResourcePrefix() + config.getStart(),
                            config.getResourcePrefix() + (config.getStart() + config.getCount() - 1));

                    for (int i = config.getStart(); i < (config.getStart() + config.getCount()); i += config
                            .getEntriesPerTransaction()) {
                        int resourceStartIndex = i;
                        int endIndex = Math.min(resourceStartIndex + config.getEntriesPerTransaction(),
                                config.getStart() + config.getCount());
                        ...
                        executor.addTaskRunningInTransaction(session -> {
                            ...
                            if (realmProvider instanceof CacheRealmProvider) {
                                ((CacheRealmProvider) realmProvider).registerRealmInvalidation(realm.getId(),
                                        realm.getName());
                            }
                    ...
                    success();
                    task.info(logger, "Created all %d resources in client %s and realm %s", context.getResourceCount(),
                            config.getClientId(), realm.getName());
                } catch (Exception ex) {
                    logException(ex);
                } finally {
                    cleanup(executor);
    ...
    private void createScopePermission(ScopeStore scopeStore, ResourceServer resourceServer, int scopeCount,
            String scopePrefix,
    ...
    private List<Policy> createUserPolicies(DatasetConfig config, ResourceServer resourceServer,
            PolicyStore policyStore, int currentResourceIndex) {
    ...
    private Resource createResource(DatasetConfig config, ResourceServer resourceServer,
        ...
        Resource resource = resourceStore.create(resourceServer, config.getResourcePrefix() + currentResourceIndex,
    ...
    private Policy createUserPolicy(DatasetConfig config, ResourceServer resourceServer, PolicyStore policyStore,
        ...
        String userName = config.getUserPrefix()
                + new Random().ints(0, config.getUsersPerRealm()).findFirst().getAsInt();
    ...
    private ResourceServer getOrCreateResourceServer(DatasetConfig config, KeycloakSession session) {
        ...
        if (resourceServer == null) {
            return KeycloakModelUtils.runJobInRetriableTransaction(session.getKeycloakSessionFactory(),
    ...
    private Task addTaskIfNotInProgress(DatasetConfig config) {
        ...
        if (client == null) {
            throw new DatasetException(
                    "Client '" + config.getClientId() + "' not found in realm '" + realm.getName() + "'");
        }

        Task task = Task.start("Creation of authorization settings in realm " + config.getRealmName() + " for client "
                + config.getClientId());
        ...
        logger.infof("Creating task to provision authorization settings for client '%s': %s", config.getClientId(),
                config);
